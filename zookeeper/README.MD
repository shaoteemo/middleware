...
Zookeeper中每一个节点类型称为**znode**节点类型，其中\ （root）为节点名称，每一个znode默认可以存储1Mb的数据，记录了zxid等元数据信息。这种节点与Unix文件系统路径相似，可以向节点存储、获取数据。
...
脑裂：同集群出现了多个Leader。在特殊情况下网络问题出现分区，导致不同区域选择了各自的Leader。
...
![zk_s_life](doc_img\zk_s_life.png)
